# Docs

| Doc | Topic |
|---|---|
| [architecture.md](architecture.md) | Topology, memory budget, pages |
| [protocol](../protocol/protocol.md) | BLE UUIDs + 16-byte packet (source of truth) |
| [hubs.md](hubs.md) | Phone vs Watch single-writer rules |
| [flash-and-restore.md](flash-and-restore.md) | Backup X4, flash BikeHUD, restore CrossPoint |
| [ci.md](ci.md) | GitHub Actions, releases, signed IPA later |
| [roadmap.md](roadmap.md) | Phased plan |
| [images/](images/) | Photos for README / docs |

## Photos

| File | Description |
|---|---|
| [images/hud-waiting.jpg](images/hud-waiting.jpg) | Desk unit, WAITING · ADV, imperial grid UI |
